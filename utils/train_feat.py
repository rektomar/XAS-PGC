...
def run_epoch_feat(model, loader, optimizer=[], verbose=False):
    nll_sum = 0.
    for b in tqdm(loader, leave=False, disable=verbose):
        x = b['x'].to(model.device) # (256,9,5)
        a = b['a'].to(model.device) # (256,9,9,4)
        y = b['y'].to(model.device).reshape(-1, 1) # (256, 1)
        def closure():
            optimizer.zero_grad()
...
def train_feat(
    ...
    for epoch in range(hyperpars['num_epochs']):
        model.train()
        nll_trn = run_epoch_feat(model, loader_trn, verbose=verbose, optimizer=optimizer)
        # scheduler.step()
        model.eval()
        ...
        if metric_type in METRIC_TYPES:
            ...
            metric = run_epoch_feat(model, loader_val, verbose=verbose)
...
def evaluate(
    ...
    if compute_nll == True:
        nll_trn_approx = run_epoch_feat(model, loader_trn)
        nll_val_approx = run_epoch_feat(model, loader_val)
        metrics_neglogliks = {
