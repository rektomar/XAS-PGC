...
from utils.train import flatten_dict, dict2str, backend_hpars_prefix
...
def run_epoch_prop(model, loader, prop_name, optimizer=[], verbose=False):
    nll_sum = 0.
    for b in tqdm(loader, leave=False, disable=verbose):
        x = b['x'].to(model.device)
        a = b['a'].to(model.device)
        y = b[prop_name].to(model.device).reshape(-1, 1)
        a = unflatt_tril(a, x.size(1))
        nll = -model.logpdf(x, a, y)
        nll_sum += nll
        if optimizer:
            optimizer.zero_grad()
            nll.backward()
...
def train_prop(
    ...
    prop_name = hyperpars['model_hpars']['prop']

    for epoch in range(hyperpars['num_epochs']):
        model.train()
        nll_trn = run_epoch_prop(model, loaders['loader_trn'], prop_name, verbose=verbose, optimizer=optimizer)
        # scheduler.step()
        model.eval()
        ...
        if metric_type in METRIC_TYPES:
            ...
            metric = run_epoch_prop(model, loaders['loader_val'], prop_name, verbose=verbose)
        ...
        if save_model == True:
            dir = base_dir + f'ckpt/{prop_name}/{hyperpars["dataset"]}/{hyperpars["model"]}/'
...
def evaluate(
    ...
    prop_name = hyperpars['model_hpars']['prop']
    path_model = base_dir + f'ckpt/{prop_name}/{hyperpars["dataset"]}/{hyperpars["model"]}/' + dict2str(flatten_dict(backend_hpars_prefix(hyperpars))) + '.pt'
    ...
    with torch.no_grad():
        if compute_nll == True:
            nll_trn_approx = run_epoch_prop(model, loaders['loader_trn'], verbose=verbose)
            nll_val_approx = run_epoch_prop(model, loaders['loader_val'], verbose=verbose)
            nll_tst_approx = run_epoch_prop(model, loaders['loader_tst'], verbose=verbose)
            metrics_neglogliks = {
    ...
    dir = base_dir + f'eval/metrics/{prop_name}/{hyperpars["dataset"]}/{hyperpars["model"]}/'
    os.makedirs(dir, exist_ok=True)
    ...
    dir = base_dir + f'eval/images/{prop_name}/{hyperpars["dataset"]}/{hyperpars["model"]}/'
    os.makedirs(dir, exist_ok=True)
