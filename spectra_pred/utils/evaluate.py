...
def eval_rse(model, loader, transform):
    rse_tensor = []
    for b in loader:
        x, a, y = b['x'].to(model.device), b['a'].to(model.device), b['spec'].to(model.device)

        y_hat = model.predict(x, a)

        y = transform.inverse(y.to('cpu'))
        y_hat = transform.inverse(y_hat.to('cpu'))

        rse_tensor.append(rse(y, y_hat))
...
def plot_results(target, prediction, metric, title=''):
    ...
    plt.subplot(1, 4, 2)
    id = 0
    plt.plot(energies, prediction[perm][id], label='Prediction', color='blue')
    plt.plot(energies, target[perm][id], label='Target', color='orange')
...
def eval_visual(model, loader, transform, title):
    ...
    for b in loader:
        ...
        prediction_tensor.append(y_hat)
